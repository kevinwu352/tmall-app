...
public class Hover {
  ...
  public var currentEntry: HoverView? {
    SwiftEntryKit.window?.find(HoverView.self)
  }
...
extension EKColor {
  @MainActor static let hoverBackground = EKColor(
  ...
  @MainActor static let hoverScreen = EKColor(
...
extension UIView {
  fileprivate func find<T: UIView>(_ cls: T.Type) -> T? {
    if self is T {
      return self as? T
    }
    for view in subviews {
      if let ret = view.find(cls) {
        return ret
      }
    }
    return nil
